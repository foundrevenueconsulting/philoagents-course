...
def test_domain_models():
    ...
    try:
        ...
        sys.path.insert(
            0,
            os.path.join(
                os.path.dirname(__file__), "src", "philoagents", "domain", "multi_way"
            ),
        )
        ...
        for config_id, config in configs.items():
            ...
            lead_agents = [
                a for a in config.agents if a.role == conversation_config.AgentRole.LEAD
            ]
            assert len(lead_agents) >= 1, (
                f"{config_id}: Must have at least one lead agent"
            )

            # Check unique IDs
            agent_ids = [a.id for a in config.agents]
            assert len(agent_ids) == len(set(agent_ids)), (
                f"{config_id}: Agent IDs must be unique"
            )
        ...
        test_dialogue = dialogue_state.DialogueState(
            session_id="test_session", config_id="business_strategy"
        )
        ...
        print(
            f"   ✅ Test dialogue state created with {len(test_dialogue.messages)} message(s)"
        )
    ...
    try:
        ...
        for config_id, config in configs.items():
            ...
            for agent in config.agents:
                ...
                print(
                    f"        Colors: {agent.primary_color} / {agent.secondary_color}"
                )
        ...
        lead_agent = business_config.get_agent_by_role(
            conversation_config.AgentRole.LEAD
        )
