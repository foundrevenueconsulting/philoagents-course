...
from philoagents.application.multi_way_conversation.conversation_orchestrator import ConversationOrchestrator
...
async def test_basic_functionality():
    ...
    session_id, dialogue_state = await orchestrator.start_conversation("business_strategy")
    ...
    dialogue_state = await orchestrator.set_topic(session_id, "Should we launch a new AI product line?")
    print(f"   ✅ Topic set: {dialogue_state.topic}")
    print(f"   ✅ Status: {dialogue_state.status.value}")
    if dialogue_state.messages:
        print(f"   ✅ Introduction message: {dialogue_state.messages[-1].content[:100]}...")
    ...
    if final_state:
        print(f"   ✅ Retrieved conversation state")
...
async def test_configuration_validation():
    ...
    for config_id, config in PREDEFINED_CONFIGURATIONS.items():
        ...
        from philoagents.application.multi_way_conversation.persona_manager import PersonaManager
