...
def test_imports():
    ...
    try:
        ...
        from philoagents.domain.multi_way.conversation_config import (
            AgentRole, ConversationFormat, AgentConfig, ConversationConfig, PREDEFINED_CONFIGURATIONS
        )
        print("   ✅ Domain models imported successfully")
        
        from philoagents.domain.multi_way.dialogue_state import (
            MessageRole, ConversationStatus, Message, TurnInfo, DialogueState
        )
        ...
        for config_id, config in PREDEFINED_CONFIGURATIONS.items():
            ...
            print(f"        Agents: {[f'{a.name} ({a.role.value})' for a in config.agents]}")
            print()
...
def test_models():
    ...
    try:
        from philoagents.domain.multi_way.conversation_config import (
            AgentRole, ConversationFormat, AgentConfig, ConversationConfig
        )
        from philoagents.domain.multi_way.dialogue_state import (
            DialogueState, ConversationStatus
        )
