...
class MultiWayConversationRepository:
        ...
        try:
            ...
            if existing:
                ...
                if existing.user_id != user_id:
                    raise ValueError(
                        f"User {user_id} does not own conversation {session_id}"
                    )
    ...
    def get_conversation_by_session_id(
        ...
        try:
            documents = self.mongo_wrapper.fetch_documents(
                limit=1, query={"session_id": session_id, "user_id": user_id}
            )
            return documents[0] if documents else None
        except Exception as e:
            logger.error(
                f"Failed to get conversation {session_id} for user {user_id}: {e}"
            )
            return None
    ...
    def get_conversation_by_id(
        ...
        try:
            ...
            documents = self.mongo_wrapper.fetch_documents(
                limit=1, query={"_id": ObjectId(conversation_id), "user_id": user_id}
            )
            return documents[0] if documents else None
        except Exception as e:
            logger.error(
                f"Failed to get conversation by ID {conversation_id} for user {user_id}: {e}"
            )
            return None
    ...
    def list_conversations(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(conversations)} conversations for user {filter_params.user_id}"
            )
    ...
    def delete_conversation(self, session_id: str, user_id: str) -> bool:
        ...
        """
        try:
            result = self.mongo_wrapper.collection.delete_one(
                {"session_id": session_id, "user_id": user_id}
            )
            ...
            logger.error(
                f"Failed to delete conversation {session_id} for user {user_id}: {e}"
            )
            return False

    def get_conversation_count(
        self, user_id: str, config_id: Optional[str] = None
    ) -> int:
        """
