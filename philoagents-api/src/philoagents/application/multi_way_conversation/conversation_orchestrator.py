...
class ConversationOrchestrator:
    ...
    async def generate_next_response(self, session_id: str) -> AsyncGenerator[Dict, None]:
        ...
        if dialogue_state.status not in [ConversationStatus.IN_PROGRESS, ConversationStatus.WAITING_FOR_USER]:
            yield {
                "type": "error",
        ...
        if not current_speaker_id:
            yield {
                "type": "error",
                "message": "Unable to determine next speaker"
            }
            return
        ...
        if not speaker_config:
            yield {
                "type": "error",
                "message": f"Agent {current_speaker_id} not found"
            }
            return
        ...
        try:
            ...
            response = await persona_manager.generate_agent_response(
                agent_id=current_speaker_id,
                dialogue_state=dialogue_state,
                last_speaker_name=last_agent_message.agent_name if last_agent_message else None
            )
            ...
            user_questions = persona_manager.extract_user_questions(current_speaker_id, response)
            ...
            if user_questions:
                dialogue_state.set_waiting_for_user(
                    prompt="; ".join(user_questions)
                )
                yield {
