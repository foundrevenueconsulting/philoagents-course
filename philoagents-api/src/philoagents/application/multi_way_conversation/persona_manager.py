...
"""
    ...
    def _initialize_agents(self) -> None:
        ...
        for agent_config in self.config.agents:
            try:
                ...
                logger.info(
                    f"Initialized agent: {agent_config.name} ({agent_config.id})"
                )
    ...
    async def generate_introduction(self, topic: str) -> str:
        ...
        if not lead_agent:
            return (
                f"Let's begin our {self.config.format.value} discussion about: {topic}"
            )
        ...
        try:
            introduction = await lead_agent.generate_response(
                conversation_history=intro_context, current_topic=topic
            )
            return introduction
        except Exception as e:
    ...
    async def determine_next_speaker(
        self, dialogue_state: DialogueState, last_speaker_id: Optional[str] = None
    ) -> Optional[str]:
        """
        ...
        if not speaking_candidates:
            ...
            return self._round_robin_selection(
                dialogue_state.active_agents, last_speaker_id
            )
    ...
    def _round_robin_selection(
        self, active_agents: List[str], last_speaker_id: Optional[str]
    ) -> Optional[str]:
