...
"""
    ...
    def should_speak_next(
        ...
        if self.config.role == AgentRole.SKEPTIC:
            agent_messages = [msg for msg in recent_messages if msg.agent_id != self.config.id]
        ...
        my_recent_count = sum(1 for msg in recent_messages if msg.agent_id == self.config.id)
    ...
    def get_conversation_summary(self, messages: List[Dict[str, str]]) -> str:
        ...
        return f"""From {self.config.name}'s perspective ({self.config.domain_expertise}):
The conversation has covered {len(messages)} exchanges. Key areas of my expertise 
({self.config.domain_expertise}) have been {"discussed" if any(
