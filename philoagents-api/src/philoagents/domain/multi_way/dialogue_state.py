...
class Message(BaseModel):
    ...
    agent_id: Optional[str] = Field(
        default=None, description="ID of agent sender (if applicable)"
    )
    agent_name: Optional[str] = Field(
        default=None, description="Name of agent sender (if applicable)"
    )
...
class TurnInfo(BaseModel):
    ...
    current_agent_id: Optional[str] = Field(
        default=None, description="ID of current speaking agent"
    )
    next_agent_id: Optional[str] = Field(
        default=None, description="ID of next speaking agent"
    )
    turn_number: int = Field(default=0, description="Current turn number")
    reasoning: Optional[str] = Field(
        default=None, description="Reasoning for agent selection"
    )
...
class DialogueState(BaseModel):
    ...
    # Turn management
    turn_info: TurnInfo = Field(
        default_factory=TurnInfo, description="Current turn information"
    )
    ...
    waiting_for_user_feedback: bool = Field(
        default=False, description="Whether waiting for user input"
    )
    user_feedback_prompt: Optional[str] = Field(
        default=None, description="Prompt for user feedback"
    )

    # Topic and conversation flow
    current_subtopic: Optional[str] = Field(
        default=None, description="Current discussion subtopic"
    )
    key_points: List[str] = Field(
        default_factory=list, description="Key points discussed"
    )
    decisions_made: List[str] = Field(
        default_factory=list, description="Decisions or consensus reached"
    )
    ...
    def add_user_message(self, content: str, message_id: str) -> None:
        ...
        for agent_id in self.active_agents:
            ...
            self.agent_contexts[agent_id].append({"role": "user", "content": content})

    def add_agent_message(
        self, agent_id: str, agent_name: str, content: str, message_id: str
    ) -> None:
        ...
        for other_agent_id in self.active_agents:
            if other_agent_id != agent_id:
                ...
                self.agent_contexts[other_agent_id].append(
                    {"role": "user", "content": f"{agent_name}: {content}"}
                )

    def add_system_message(self, content: str, message_id: str) -> None:
        """Add a system message to the conversation"""
        message = Message(id=message_id, role=MessageRole.SYSTEM, content=content)
        self.add_message(message)
        ...
        for agent_id in self.active_agents:
            ...
            self.agent_contexts[agent_id].append({"role": "system", "content": content})
