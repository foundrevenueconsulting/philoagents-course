...
class PersistedConversation(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=datetime.now, description="When conversation was created"
    )
    updated_at: datetime = Field(
        default_factory=datetime.now, description="Last update time"
    )
    ended_at: Optional[datetime] = Field(
        default=None, description="When conversation ended"
    )
    ...
    def update_stats(self) -> None:
        ...
        self.user_messages = len(
            [m for m in self.dialogue_state.messages if m.role == "user"]
        )
        self.agent_messages = len(
            [m for m in self.dialogue_state.messages if m.role == "agent"]
        )
...
class ConversationListFilter(BaseModel):
    ...
    config_id: Optional[str] = Field(
        default=None, description="Filter by configuration ID"
    )
