...
export default function ImageRecognitionPracticePage() {
  ...
  const loadNewQuestion = async () => {
    ...
    try {
      ...
      if (lastQuestionId) {
        url += `?exclude_item_id=${encodeURIComponent(lastQuestionId)}`;
      }
      ...
      if (response.ok) {
        ...
        setQuestion(data);
        setLastQuestionId(data.id); // Track this question for next time
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={loadNewQuestion} variant="outline" size="sm" disabled={isLoading}>
            🔄
            New Question
          </Button>
        </div>
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-600">
              Total: {userStats.total_attempts} • Accuracy: {userStats.accuracy_percentage.toFixed(1)}%
            </div>
            <div className="text-sm text-gray-600">
              Avg Time: {(userStats.average_response_time_ms / 1000).toFixed(1)}s
            </div>
          </div>
        </div>
      )}
